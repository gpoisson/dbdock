...
def getRandomPrediction(predictions, deltaGs, lig_feature_data):
	...
	lf = None
	p = None
	if len(lig_feature_data) > 0:
		lf = lig_feature_data[rand_index] 
	if len(predictions[rand_index] > 0):
		p = predictions[rand_index]
	return lf, p
...
def finalPredictions(model):
	...
	for lig in lig_feature_data:
		test_data.append(lig[1])
	print "Predicting {} delta Gs...".format(len(test_data))
	predictions = model.predict(test_data)
	known_dgs = []
	for lig in lig_feature_data:
		known_dgs.append(getDeltaG(lig))
	error = np.asarray(known_dgs - predictions)
	np.save("model_error_unk_samples_c_{}.npy".format(c_val),error)
...
def main():
	...
	next_set = naive_set
	current_layers = 0
	while (current_layers < sample_layers):
		this_set = next_set
